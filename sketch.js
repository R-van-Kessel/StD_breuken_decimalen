...
function generateQuestions() {
    ...
    // Veel voorkomende breuken met hun decimale waarden
    const commonFractions = [
        ...
        { numerator: 3, denominator: 10, decimal: 0.3 },
        { numerator: 7, denominator: 10, decimal: 0.7 },
        { numerator: 9, denominator: 10, decimal: 0.9 },
        { numerator: 1, denominator: 20, decimal: 0.05 },
        { numerator: 3, denominator: 20, decimal: 0.15 },
        { numerator: 7, denominator: 20, decimal: 0.35 },
        { numerator: 9, denominator: 20, decimal: 0.45 },
        { numerator: 11, denominator: 20, decimal: 0.55 },
        { numerator: 13, denominator: 20, decimal: 0.65 },
        { numerator: 17, denominator: 20, decimal: 0.85 },
        { numerator: 19, denominator: 20, decimal: 0.95 }
    ];

    for (let i = 0; i < 10; i++) {
        // Kies random breuk
        const fraction = commonFractions[floor(random(commonFractions.length))];
        
        // 50% kans: breuk naar decimaal, 50% kans: decimaal naar breuk
        const questionType = random() > 0.5 ? 1 : 2;
        
        let text, answer;
        
        if (questionType === 1) {
            // Breuk naar decimaal — teller boven streep, noemer eronder
            text = `${fraction.numerator}\n──\n${fraction.denominator}\n= ?`;
            answer = `${fraction.decimal}`.replace('.', ',');
            
        } else {
            // Decimaal naar breuk
            text = `${fraction.decimal}`.replace('.', ',') + `\n ?\n──\n?`;
...
function drawBlock(block) {
  ...
  let streepIndex = lines.findIndex(l => l.includes('──'));

  if (streepIndex > 0) {
    // Heeft een breukstreep
    let teller = lines[streepIndex - 1].trim();
    let noemer = lines[streepIndex + 1] ? lines[streepIndex + 1].trim() : '';
    
    // Alles na de noemer is de "= ?" of "= decimaal" tekst
    let naTekst = lines.slice(streepIndex + 2).join(' ').trim();
    
    // Alles vóór de teller is bijv. een decimaal getal
    let voorTekst = lines.slice(0, streepIndex - 1).join(' ').trim();

    let streepBreedte = 20;
    let gapY = 6;  // afstand cijfer tot streep

    // Breuk staat links, "= ?" rechts ernaast
    // Bereken breedte breukgedeelte om goed te centreren samen met naTekst
    textSize(16);
    textSize(16);
    let voorTextBreedte = voorTekst !== '' ? textWidth(voorTekst + ' = ') : 0;
    let naTextBreedte = naTekst !== '' ? textWidth(' ' + naTekst) : 0;
    let totaalBreedte = voorTextBreedte + streepBreedte + naTextBreedte;
    let startX = cx - totaalBreedte / 2 - 15;  // ← getal verhogen = meer naar links
    let breukX = startX + voorTextBreedte + 20; // ← getal verhogen = meer naar rechts

    // Decimaal links naast de breuk
    if (voorTekst !== '') {
      textAlign(LEFT, CENTER);
      textSize(16);
      text(voorTekst + '  =', startX, cy);
    }

    // Teller boven streep
    textAlign(CENTER, BOTTOM);
    textSize(16);
    text(teller, breukX, cy - gapY);

    // Breukstreep
    stroke(0);
    strokeWeight(2);
    line(breukX - streepBreedte / 2, cy, breukX + streepBreedte / 2, cy);
    noStroke();
    ...
    textAlign(CENTER, TOP);
    textSize(16);
    text(noemer, breukX, cy + gapY);

    // "= ?" rechts naast de breuk, verticaal gecentreerd
    if (naTekst !== '' && voorTekst === '') {
      textAlign(LEFT, CENTER);
      textSize(15);
      text(' = ?', breukX + streepBreedte / 2 + 2, cy);
    }

  } else {
    // Geen breuk: gewone tekst gecentreerd
    textAlign(CENTER, CENTER);
    textSize(16);
    textLeading(25);
